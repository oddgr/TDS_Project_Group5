...
ethnic <- as.factor(mydata$ethnicity)
label(ethnic) <- "Ethnic"
mydata$ethnic <- ethnic

mydata$outcome_status <- factor(mydata$outcome_status, levels = c(0, 1, 2), labels = c("Control", "Case", "P-value"))

tb1 <- table1(~ sex + age_at_recuritment + ethnic | outcome_status, data = mydata, droplevels = F, render=rndr, render.strat=rndr.strat)
```
...
mydata$waist_circumference <- waist_circumference

hip_circumference <- mydata$X49.0.0
label(hip_circumference) <- "Hip circumference"
mydata$hip_circumference <- hip_circumference
...
```{r}
tb_full <- table1(~ sex + age_at_recuritment + ethnic + bmi + waist_circumference + hip_circumference + smoke + alcohol+ active_days + Cardiovascular + Diabetes + Hypertension + Respiratory + breast_cancer_screening + menarche_age  + had_menopause + number_of_live_births + ever_have_birth + termination + oral_contraceptive + hrt| outcome_status, data = mydata, droplevels = F, render=rndr, render.strat=rndr.strat)
```